...
<head>
  ...
  <body background=".././Recursos/img/fondo.jpg">
      ...
      <li><a href='.././login/logout.php'>Cerrar Sesion</a></li>
            ...
            <form id="f1" name="f1" method="post" onsubmit="return validar();" action="enviardatoseditarhabitaciones.php" width="150" height="500">
              <div class "mt">
                ...
                echo "<pre class='text-uppercase text-sm'>Habitacion a editar: </pre>";
                echo "<select name='nombreantiguo'>";

                while ($row = mysql_fetch_array($query)){
                echo "<option name='todainformacion' class='form-control mb' value='" .$row['nombre']."'>".$row['idHabitacion'] .' ||| Nombre ->'.$row['nombre']."</option>";
                    }
                echo "</select>";
                echo "</form>";
                ?>

              <label for="" class="text-uppercase text-sm">Nuevo nombre de la habitacion:</label>
              <input type="text" placeholder="Escriba el nuevo nombre" id='nombrenuevo' name="nombrenuevo" class="form-control mb" pattern="[a-zA-Zñ ]+[a-zA-Zñ]{1,15}" required=""/>

              <label for="" class="text-uppercase text-sm">Nuevo ID de casa asociada:</label>
              <input type="text" placeholder="Escriba el ID" id='nuevonombrecasa' name="nuevonombrecasa" class="form-control mb" pattern="{1,15}" required=""/>
