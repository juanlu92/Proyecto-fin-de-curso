...
$_SESSION['privilegiosfinal']=$clave3;

$sql1 = "SELECT idUsuario, email FROM Usuarios";
							//--------------------COGE DATOS E INSERTA---------------------//

$query=mysql_query($sql1) or die('Error en la consulta: ' . mysql_error());

$numerodefilas=mysql_num_rows($query);
echo "<form method='post' action='relacionar_habitacion_componente.php'>";
echo "<pre class='form-control mb'>Usuario a asociar: </pre>";
echo "<select name='iddeusuario'>";

while ($row = mysql_fetch_array($query)){
echo "<option name='todainformacion' class='form-control mb' value='" .$row['idUsuario']."'>".$row['idUsuario'] .' ||| Nombre ->'.$row['email']."</option>";
		}
